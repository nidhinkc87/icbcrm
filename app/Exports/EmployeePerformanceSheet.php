...
class EmployeePerformanceSheet implements FromArray, ShouldAutoSize, WithTitle, WithStyles
{
    public function __construct(private array $data) {}

    public function title(): string
    {
        return substr(preg_replace('/[^\w\s-]/', '', $this->data['employee']['name']), 0, 31);
    }
    ...
    public function array(): array
    {
        ...
        $rows[] = [];
        $rows[] = ['Recent Completed Tasks'];
        $rows[] = ['#', 'Service', 'Customer', 'Priority', 'Due Date', 'Completed', 'Days Taken', 'Status'];

        foreach ($this->data['recent_tasks'] as $i => $t) {
            $rows[] = [
                $i + 1,
                $t['service_name'],
                $t['customer_name'],
